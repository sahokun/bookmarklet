(function(){try{let e=document.querySelector(".unit__heroImage--original img");e||=document.querySelector(".unit__heroImage img");const m=e.getAttribute("src").match(/\/([^\/]+)\.png$/),n=parseInt(m[1]),p=Array.from(document.querySelectorAll(".unit__skills.unit__section .unit__section__activeSkileName")).map(a=>a.textContent),q=window.__NUXT__.state.skill.masters,r=window.__NUXT__.state.extension.masters,g=p.map(a=>{const b=q.find(f=>f.name.ja===a||f.name.en===a).skillId,c=r.find(f=>f.active===b);
