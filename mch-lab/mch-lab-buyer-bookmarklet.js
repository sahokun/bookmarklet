javascript: (async function () {
  ...
  function setText(content) {
    ...
    const existingPElement = targetDiv.querySelector(
      "p.optimal-purchase-amount"
    );
  ...
  async function findOptimalPurchaseAmount(materialId, requiredAmount) {
    ...
    var upperBound =
      Math.ceil(
        ((Math.ceil(initialPrice * requiredAmount) + 1) / initialPrice) *
          SCALE_FACTOR
      ) / SCALE_FACTOR;
    ...
    // 希望購入素材量での価格取得
    var minEstimate =
      await window.$nuxt.$store.$laboratoryService.estimatePurchaseAmount(
        materialId,
    ...
    while (true) {
      ...
      // 中間値の計算
      let mid =
        Math.ceil(((lowerBound + upperBound) / 2) * SCALE_FACTOR) /
        SCALE_FACTOR;
      // 中間値での価格取得
      let midEstimate =
        await window.$nuxt.$store.$laboratoryService.estimatePurchaseAmount(
  ...
  try {
    ...
    // 計算
    let optimalPurchaseAmount = await findOptimalPurchaseAmount(
      material.materialId,
      requiredAmount
    );
    // 出力
